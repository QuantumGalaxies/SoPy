...
def test_gaussian_overlap_integral():
    ...
    expected_value = mathematica_ground_truth(alpha1, alpha2,0)**2
    
    # --- ACT ---
    # alpha = 1/sigma**2
    sigma1 = 1/np.sqrt(alpha1)
    sigma2 = 1/np.sqrt(alpha2)
    lattices = 2*[ np.linspace( - 4*max(sigma1,sigma2), 4 * max(sigma1,sigma2), 1000)]
    
    u1 = sp.Vector().gaussian(a = 1, \
      positions=[0.,0.], \
      sigmas = [ np.sqrt(alpha1),np.sqrt(alpha1)], \
      ls = [0,0], \
      lattices = lattices)
    
    u2 = sp.Vector().gaussian(a = 1, \
      positions=[0.,0.], \
      sigmas = [ np.sqrt(alpha2), np.sqrt(alpha2)], \
      ls = [0,0], \
      lattices = lattices)
...
def test_gaussian_delta_integral():
    ...
    expected_value = compute(spacing2, 0, alpha1, 0, position)**2
...
def test_ft_gaussian_overlap_integral():
    ...
    # Get the "Golden Standard" from your Mathematica equation
    expected_value = np.real(mathematica_ground_truth(alpha1, alpha2,k))**2
    
    # --- ACT ---
    # alpha = 1/sigma**2
    sigma1 = 1/np.sqrt(alpha1)
    sigma2 = 1/np.sqrt(alpha2)
    lattices = 2*[ np.linspace( - 20*max(sigma1,sigma2),20 * max(sigma1,sigma2), 2000)]
    ...
    simulated_sopy_result = (u1.exp_i(ks=[k,k]).re.dot(u2))
